...
const CustomizePanel = ({ isOpen, onClose, settings, onSettingsChange }) => {
  const [selectedFiles, setSelectedFiles] = useState(settings.selectedFiles || []);
  const [topK, setTopK] = useState(settings.topK || 5);
  const [chatModel, setChatModel] = useState(settings.chatModel || 'gpt-4');
    ...
    <div className="w-80 bg-white border border-gray-200 flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        ...
        <div>
          ...
          >
            <option value="" disabled>Choose files...</option>
            {availableFiles.map(file => (
              <option key={file} value={file} disabled={selectedFiles.includes(file)}>
                {file}
              </option>
            ))}
          </select>
